module.exports = app =>{
    const { existOrError, notExistOrError,stringOverflowError} = app.api.validation
    ...
    const save = (req, res) => { 
        ...
        try{
            ...
            stringOverflowError(category.name,200,'Limite alcançado')
        }  catch(msg){
        ...
        if(category.id) {
            ...
            app.db('categories')
            .insert(category).then( _ => res.status(204).send())
            .catch(err => res.status(500).send(err))
        }
    }
    ...
    const remove = async (req, res) => {
        try {
            ...
            notExistOrError(articles,'Categoria existe artigos.')
